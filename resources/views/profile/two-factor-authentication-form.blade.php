<x-jet-action-section>
    <x-slot name="title">
        {{ __('Factor de Autenticación Adicional ') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Añade seguridad adicional a tu cuenta usando 2 factores de autenticación.') }}
    </x-slot>

    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                {{ __('Has establecido 2 factores de autenticación.') }}
            @else
                {{ __('No has establecido 2 factores de autenticación.') }}
            @endif
        </h3>
